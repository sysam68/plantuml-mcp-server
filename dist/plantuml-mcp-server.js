...
import { CallToolRequestSchema, CompleteRequestSchema, GetPromptRequestSchema, ListPromptsRequestSchema, ListResourceTemplatesRequestSchema, ListResourcesRequestSchema, ListToolsRequestSchema, ReadResourceRequestSchema, SetLevelRequestSchema, isInitializeRequest, } from '@modelcontextprotocol/sdk/types.js';
...
let obFileOidcMetadataCache;
let obFileAccessTokenCache;
...
function hasObFileOidcClientCredentials() {
    return Boolean((OB_FILE_OIDC_DISCOVERY_URL || OB_FILE_OIDC_TOKEN_ENDPOINT) &&
        OB_FILE_OIDC_CLIENT_ID &&
        OB_FILE_OIDC_CLIENT_SECRET);
}
function buildObFileOidcHeaders() {
    const headers = {};
...
async function getObFileOidcMetadata() {
...
function requireObFileTokenEndpoint(metadata) {
    if (!metadata.token_endpoint) {
        throw new Error('OIDC discovery response missing token_endpoint');
    }
    return metadata.token_endpoint;
}
async function getObFileApiAccessToken() {
...
function ensureHttpCorsHeaders(res) {
    res.setHeader('Access-Control-Allow-Origin', '*');
    res.setHeader('Access-Control-Allow-Headers', 'content-type, authorization, mcp-session-id');
    res.setHeader('Access-Control-Expose-Headers', 'mcp-session-id');
    res.setHeader('Vary', 'Origin');
}
function sendJsonError(res, statusCode, message, code = -32000) {
    if (res.headersSent) {
        res.end();
        return;
    }
...
function isInitializationPayload(payload) {
    ...
    try {
        ...
        if (OB_FILE_API_BASE_URL) {
            try {
                ...
                uploadForm.set('file', new Blob([new Uint8Array(content)], { type: `image/${format}` }), fileName);
                ...
                const publicUrlResponse = await fetch(`${OB_FILE_API_BASE_URL}/api/files/${encodeURIComponent(fileName)}/public-url`, {
                    method: 'POST',
                    headers: requestHeaders,
                });
                if (!publicUrlResponse.ok) {
                    throw new Error(`ob-file public-url failed with ${publicUrlResponse.status}: ${await publicUrlResponse.text()}`);
                }
                const publicUrlPayload = (await publicUrlResponse.json());
...
export class PlantUMLMCPServer {
    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            this.log('debug', `CallTool response for ${name}:\n${stringifyForLog(response?.structuredContent ?? response)}`);
            return response;
        });
    }
    setupPromptHandlers() {
    ...
    async generateDiagram(args) {
        ...
        try {
            ...
            if (storedDiagram) {
                this.log('debug', storedDiagram.filePath
            ...
            if (storedDiagram) {
                contentParts.splice(2, 0, storedDiagram.filePath
...
async function startStreamableHttpServer() {
                ...
                if (req.method === 'POST') {
                    ...
                    if (!session) {
                        ...
                        sessionRecord.instance.onError((error) => {
                            serverInstance.log('error', `Unhandled error in HTTP session ${sessionRecord.transport.sessionId ?? 'pending'}`, error);
                        });
                        sessionRecord.transport.onerror = (error) => {
                            serverInstance.log('error', `Unhandled error in streamable HTTP transport ${sessionRecord.transport.sessionId ?? 'pending'}`, error);
                        };
    ...
    await new Promise((resolve, reject) => {
        ...
        httpServer.listen(MCP_PORT, MCP_HOST, () => {
            logToConsole('info', `PlantUML MCP server (HTTP transport) listening on http://${MCP_HOST}:${MCP_PORT}${MCP_HTTP_PATH}`);
        });
        const shutdown = async () => {
            logToConsole('info', 'Shutdown signal received, closing HTTP server.');
            try {
                await Promise.all(Array.from(sessions.values()).map(async ({ transport, instance }) => {
                    try {
